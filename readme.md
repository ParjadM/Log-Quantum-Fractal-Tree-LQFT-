...
- v1.2.2 now exposes one intended high-level model, `LQFTMap`, while retaining `LQFT` and `MutableLQFT` as compatibility surfaces.
- v1.2.2 is a substantially better mutable/write-heavy LQFT than earlier releases.
- v1.2.2 also has a materially stronger mutable-to-persistent commit path for snapshot-oriented workloads, especially when deletes are part of the delta.
- v1.2.2 is not a proof that LQFT beats common in-memory data structures overall.

---

## Performance Snapshot (v1.2.2)
