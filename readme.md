...
- v1.1.6 now exposes one intended high-level model, `LQFTMap`, while retaining `LQFT` and `MutableLQFT` as compatibility surfaces.
- v1.1.6 is a substantially better mutable/write-heavy LQFT than earlier releases.
- v1.1.6 also has a materially stronger mutable-to-persistent commit path for snapshot-oriented workloads, especially when deletes are part of the delta.
